...
public class PainelLivro extends JPanel {
    ...
    private void carregarTabela() {
        ...
        if (!alertas.isEmpty()) {
            ...
            for (Livro livro : alertas) {
                sb.append("- ").append(livro.getTitulo()).append(" (Estoque: ").append(livro.getQuantidadeEstoque()).append(")\n");
            }
    ...
    private Livro criarLivroDoFormulario(boolean isModificacao) {
        try {
            ...
            JOptionPane.showMessageDialog(this, "Erro de formato numérico. Verifique Preço, Edição, Estoque e Estoque Mínimo.", "Erro", JOptionPane.ERROR_MESSAGE);
            return null;
        } catch (Exception ex) {
    ...
    private void modificarLivro() {
        if (campoId.getText().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Selecione um livro na tabela ou preencha o ID para modificar.", "Erro", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        if (livroModificado != null) {
            try {
                ...
                JOptionPane.showMessageDialog(this, "Livro modificado com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
                limparCampos();
                carregarTabela();
            } catch (IllegalArgumentException ex) {
                JOptionPane.showMessageDialog(this, ex.getMessage(), "Erro de Negócio", JOptionPane.ERROR_MESSAGE);
    ...
    private void apagarLivro() {
        if (campoId.getText().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Selecione um livro na tabela ou preencha o ID para apagar.", "Erro", JOptionPane.ERROR_MESSAGE);
            return;
        }
        try {
            int id = Integer.parseInt(campoId.getText());
            int confirmacao = JOptionPane.showConfirmDialog(this, "Tem certeza que deseja apagar o livro ID: " + id + "?", "Confirmação", JOptionPane.YES_NO_OPTION);
            if (confirmacao == JOptionPane.YES_OPTION) {
                servicoLivro.apagar(id);
                JOptionPane.showMessageDialog(this, "Livro apagado com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
                limparCampos();
                carregarTabela();
            }
        } catch (NumberFormatException ex) {
        ...
        if (idStr != null && !idStr.trim().isEmpty()) {
            try {
                ...
                if (livro != null) {
                    ...
                    JOptionPane.showMessageDialog(this, "Livro com ID " + id + " não encontrado.", "Consulta", JOptionPane.INFORMATION_MESSAGE);
                    limparCampos();
                }
            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(this, "ID inválido.", "Erro", JOptionPane.ERROR_MESSAGE);
